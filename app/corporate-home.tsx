...
const insights=[
  ...
  ["方法论", "从零散市场信息到可信商业情报", "介绍来源分级、字段标准化、交叉核验和持续更新的研究流程。", "2026.08.10", "insight-c"],
];
...
export default function CorporateHome(){
  ...
  return <div className="corp-site">
    <header className="corp-nav"><a className="corp-logo" href="#home"><span>枢</span><b>因恒科技</b><small>CENTRAL ASIA INTELLIGENCE</small></a><button className="menu-toggle" aria-label="打开导航" aria-expanded={menu} onClick={()=>setMenu(!menu)}>☰</button><nav className={menu?"open":""}>{[["#home","首页"],["#products","产品与服务"],["#coverage","数据覆盖"],["#solutions","解决方案"],["#insights","市场洞察"],["#about","关于我们"]].map(([href,label])=><a href={href} key={href} onClick={()=>setMenu(false)}>{label}</a>)}</nav><button className="lang-switch" onClick={()=>setLang(lang==="中文"?"EN":"中文")} aria-label="切换语言">{lang}⌄</button><a className="nav-demo" href="#contact">申请演示</a></header>
    <main>
      <section className="corp-hero" id="home"><div className="corp-hero-copy"><span>YINHENG · CENTRAL ASIA INTELLIGENCE</span><h1>洞察中亚市场，<br/>连接数据与决策</h1><p>汇集区域宏观经济、产业、企业与市场信息，为金融机构、投资者和企业提供可信、及时、可执行的商业情报。</p><div><a className="corp-primary" href="#contact">申请产品演示</a><a className="corp-secondary" href="#solutions">了解解决方案 →</a></div></div><div className="corp-hero-visual"><Image src="/corporate-hero.png" alt="中亚城市、商业与区域连接的品牌视觉" width={1536} height={1024} priority/><div className="visual-caption"><span>REGIONAL FOCUS</span><b>中国 · 喀什 · 中亚五国</b></div></div></section>

      <section className="value-section"><div className="corp-section-head"><span>CORE VALUE</span><h2>从分散信息到可执行判断</h2><p>以区域专注、多源整合和严谨研究，降低中亚市场的信息获取与验证成本。</p></div><div className="value-grid">{[["整","区域数据整合","统一整理宏观、贸易、产业和市场信息，保留来源与统计口径。"],["监","市场持续监测","跟踪价格、渠道、政策和项目变化，形成连续的市场观察。"],["研","企业与产业研究","围绕重点行业、企业和产业链建立结构化研究框架。"],["策","决策情报服务","将数据转化为简报、专题报告和可执行的研究建议。"]].map(([icon,title,copy])=><article key={title}><i>{icon}</i><h3>{title}</h3><p>{copy}</p><a href="#products">了解更多 →</a></article>)}</div></section>

      <section className="product-section" id="products"><div className="corp-section-head"><span>PRODUCTS & SERVICES</span><h2>服务中亚研究与跨境决策</h2><p>当前阶段以数据整合、区域研究和定制服务为核心，专业情报终端将在后续独立推出。</p></div><div className="corp-product-grid">{products.map(([tag,title,copy],i)=><article key={title}><span>0{i+1} / {tag}</span><h3>{title}</h3><p>{copy}</p><a href="#contact">咨询产品 →</a></article>)}</div></section>
    ...
    <footer className="corp-footer"><div className="footer-brand"><span className="footer-seal">枢</span><div><b>因恒科技</b><p>面向国际投资者、金融机构、企业管理者和研究人员的中亚金融与商业情报平台。</p></div></div><div><b>产品</b><a href="#products">市场信息</a><a href="#products">区域数据库</a><a href="#products">研究与咨询</a></div><div><b>解决方案</b><a href="#solutions">金融机构</a><a href="#solutions">跨境投资者</a><a href="#solutions">国际贸易企业</a></div><div><b>联系</b><a href="#contact">申请演示</a><a href="#contact">联系我们</a><a href="#insights">市场洞察</a></div><div><b>法律</b><a href="/privacy">隐私政策</a><a href="/terms">使用条款</a><span>情报终端 · 即将推出</span></div><div className="footer-bottom"><span>© 2026 因恒科技</span><span>公开信息仅供研究参考，不构成投资或交易建议。</span></div></footer>
  </div>
}
