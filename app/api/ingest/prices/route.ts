...
export async function POST(request:Request){
 ...
 const expected:Record<string,string>={KZ:"KZT",UZ:"UZS",KG:"KGS",TJ:"TJS",TM:"TMT"},allowed=new Set(["KZT","UZS","KGS","TJS","TMT","USD"]);
 ...
 for(const [index,x] of body.items.entries()){
  ...
  const reason=!x.platform||!x.platform_product_id||!x.product_url||!x.original_title||!x.country||!x.city?"missing required field":!allowed.has(x.currency)?"illegal currency":x.current_price==null||x.current_price<=0?"price must be positive":expected[x.country]&&expected[x.country]!==x.currency?"currency-country mismatch":null;
  ...
  try{
   const stmts=[
    ...
    db.prepare("INSERT INTO collection_points(id,country,city,platform_id,timezone,active,public_label) VALUES(?,?,?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET active=1").bind(x.collection_point_id,x.country,x.city,x.platform,"Asia/Almaty",1,x.city),
