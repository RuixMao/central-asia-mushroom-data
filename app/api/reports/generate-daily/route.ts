...
const forbidden=/https?:\/\/|(?:price|observed|source)\s*[_-]\s*(?:usd|local|cny|at|url)|\b(?:AI|API|JSON|LLM|GPT|ChatGPT|DeepSeek|SQL|D1|null|live|gap|prompt|price_retail)\b|人工智能|大模型|语言模型|模型生成|智能生成|自动生成|机器生成|算法生成|数据库|字段|代码|键值|请求|响应|自动采集|采集管线|抓取|爬虫|入库|接口|算法|样本有限|报价有限|数据有限|仅作参考|仅供参考|自动复核|已核验材料|检索到\s*\d+\s*条材料|经核验|有效参考价|有效报价|有效观察|不作外推|不据此判断整体涨跌|不直接推导批发利润|本报告暂不|给老板|给采购|给外贸|来源与资料日期/iu;
const sections=["今日要点","市场动态","机会与风险","行动建议","数据说明"];
const customerSafe=(body:string,allowed:Set<string>)=>{const normalized=body.normalize("NFKC"),refs=[...normalized.matchAll(/\[(S\d+)\]/g)].map(match=>match[1]);return normalized.length>=500&&normalized.length<=2500&&!normalized.includes("```")&&!normalized.includes("食用菌出海市场日报｜")&&!forbidden.test(normalized)&&sections.every(section=>normalized.split(section).length===2)&&refs.every(ref=>allowed.has(ref))};
...
export async function POST(){
 ...
 const prompt=`你是因恒科技的中亚及东南亚食用菌市场研究日报主编，面向企业老板、外贸与采购负责人撰写3分钟内可读完的B端商业日报。只可使用下方价格、同商品历史序列和已核验证据，不得虚构数据、事件、因果、需求、利润或预测。\n日期：${date}\n价格明细：\n${table}\n同商品历史序列：${JSON.stringify(trends)}\n年度进口单价参考（贸易口径，UN Comtrade）：${JSON.stringify(annualRef)}\n已核验外部证据：${JSON.stringify(evidence.map(item=>item.context))}`;
 const marketScope=`市场范围为中亚五国及东南亚的老挝、越南、泰国、缅甸、柬埔寨。老挝是东南亚首要拓展市场；仅在当日事实满足发布门禁时优先呈现，不得预设正面结论。`;
 const v44=`最高优先级执行公众号日报v4.4成稿规范：给客户看成品，不呈现采集、复核、检索、材料数量、样本统计、数据质量自评或内部方法。正文恰好使用“今日要点、市场动态、机会与风险、行动建议、数据说明”五个二级标题。今日要点3至5条，每条包含结论、数字或事实依据和是否行动。市场动态按鲜品、干品、冷冻、腌渍分表，品类使用中文规范名；不同规格分行，精品或小包装标注规格。无事件写“今日无新增政策与事件，市场面平稳”。机会与风险必须说明国家、品类、触发信号、损失时点和规避动作。行动建议固定为“决策参考、采购落地、报价规范”，三项含义不重叠，并引用当日国家、品类、渠道和数字。最后仅写一次：数据说明：本报告价格来自中亚及东南亚目标市场主流零售与电商渠道公开挂牌价，统一折算为美元/公斤。零售挂牌价与批发成交价、到岸成本存在差异，正式决策请以批量报价为准。数据来源：因恒科技监测，采集日期 ${date}。如需核验具体报价来源，可联系专属客服索取。禁止独立“来源与资料日期”章节。输出标准Markdown正文，不输出日报标题。`;
 let analysis="";
 for(let attempt=0;attempt<2;attempt++){
   const request=attempt?`${marketScope}\n\n${prompt}\n\n${v44}\n\n上一稿未通过发布检查，请严格按v4.4五个栏目完整重写。`:`${marketScope}\n\n${prompt}\n\n${v44}`;
 ...
 const body=analysis,title=`食用菌出海市场日报｜${Number(month)}月${Number(day)}日：市场平稳无异常`,id=crypto.randomUUID(),slug=`${date}-mushroom-export-market-research-${id.slice(0,6)}`,now=new Date();
