...
export async function POST(){
 ...
 const prompt=`你是因恒科技的中亚食用菌首席市场研究员。请写面向进口商、渠道商、投资人与经营管理层的中文决策简报。客户为减少验证成本和错误决策付费，不为报价复述或通用建议付费。只可使用下方价格、同商品历史序列和证据包，不得自行补充新闻、政策、数字、来源、因果、利润或预测。\n日期：${date}\n价格明细：\n${table}\n同商品历史序列：${JSON.stringify(trends)}\n年度进口单价参考（贸易口径，UN Comtrade）：${JSON.stringify(annualRef)}\n已核验外部证据：${JSON.stringify(evidence.map(item=>item.context))}\n\n成稿要求：\n1. 先写50至90字导读；随后恰好使用“今日结论”“可执行信号”“验证清单”“商业边界”“数据口径”五个二级标题。正文500至1200字；没有新增信号时宁可短写，不得凑篇幅。\n2. 今日结论最多3条，只回答出现了什么、是否达到行动门槛、客户现在应做或不应做什么；严禁逐国罗列报价、中位数和渠道数。\n3. 可执行信号只能引用同一商品连续变化，写明国家、品类、变化、证据强度、商业意义和停止条件。没有达到门槛时明确写“今日无新增可执行信号”，不得制造商机。\n4. 验证清单列出在售与促销状态、净重、等级、产地、最小订货量、报价有效期、物流、损耗、税费和渠道费用；不得使用“持续关注”“加强合作”等空话。\n5. 商业边界必须区分挂牌价、成交价、到岸成本、需求和利润。没有真实询盘、批量报价和完整成本时，不得给出利润、需求增长或扩产结论。\n6. 政策或新闻事实必须引用 [S1] 形式的证据编号，且只能使用证据包存在的编号。\n7. 不得出现任何生成方式、内部系统、技术字段或流程词。输出 Markdown 正文，不附来源清单或网址。`;
 let analysis="";
