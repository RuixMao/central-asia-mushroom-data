...
export default function MarketDataPage() {
  return <div className="data-site">
    ...
    <main>
      ...
      <section className="data-block" id="price"><div className="data-heading"><div><span>03 · MARKET PRICE</span><h2>市场挂牌价格样例</h2></div><p>挂牌价并非实际成交价，用于观察区域价格带与渠道差异。</p></div><div className="price-grid">{prices.map(([city, item, channel, price, date]) => <article key={city + item + channel}><span>{city} · {channel}</span><h3>{item}</h3><strong>{price}</strong><small>采集期 {date}</small></article>)}</div></section>
