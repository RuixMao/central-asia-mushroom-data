...
const countries = countryOptions.filter(country => country.code !== "ALL");
const money = (value: number | null) => value === null ? "未报告" : `$${value.toLocaleString("en-US")}`;
const gapRate = (importer: number | null, exporter: number) => importer === null ? null : Math.abs(importer - exporter) / Math.max(importer, exporter) * 100;
...
export default function MarketDataPage() {
  return <ProductShell><div className="data-site">
    <main>
      ...
      <section className="data-block pale" id="mirror"><div className="data-heading"><div><span>02 · MIRROR CHECK</span><h2>中国出口 × 中亚进口交叉核验</h2></div><p>先按中亚五国分组，再按菌类品类分别核验。镜像差异用于发现统计口径、转口与链路问题，不直接代表损失或市场空间。</p></div><div className="mirror-country-list">{countries.map(country => { const records = mirrorRecords.filter(record => record.countryCode === country.code); return <section className="mirror-country" key={country.code}><header><div><b>{country.label}</b><span>{country.code}</span></div><small>{records.length ? `${records.length} 个可核验品类` : "暂无可核验品类"}</small></header>{records.length ? <div className="mirror-grid">{records.map(record => { const gap = gapRate(record.importerCifUsd, record.chinaFobUsd); const status = gap === null ? "进口方数据待补" : gap >= 70 ? "显著差异" : gap >= 30 ? "需进一步核验" : "低基数观察"; return <article key={record.countryCode + record.hs}><span className="mirror-category">{record.product}</span><h3>HS {record.hs}</h3><div><p>进口方 CIF<b>{money(record.importerCifUsd)}</b></p><p>中国出口 FOB<b>{money(record.chinaFobUsd)}</b></p></div><strong>{gap === null ? "—" : `${gap.toFixed(1)}%`}<small>镜像差异率</small></strong><i>{status}</i></article>; })}</div> : <div className="mirror-empty"><b>数据采集中</b><p>本期尚无同时满足国别、HS 品类和双边口径要求的镜像记录。</p></div>}</section>; })}</div><p className="data-footnote">差异率 = |进口方 CIF − 中国出口 FOB| ÷ max（两者）；“未报告”表示来源缺值，不以 0 替代。</p></section>
