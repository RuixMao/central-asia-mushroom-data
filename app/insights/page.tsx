...
export default function Page() {
  return (
    <ProductShell>
      <main className="saas-main insight-landing">
        <section className="saas-hero compact">
          <span>DEMAND INSIGHTS</span>
          <h1>食用菌出海需求分析</h1>
          <p>
            以贸易规模、渠道价格与供应结构为核心，持续评估中亚与东南亚目标市场的品类需求和进入机会。
          </p>
        </section>
        <section className="insight-brief" aria-label="研究摘要">
          ...
          <div>
            <span>核心口径</span>
            <strong>3 类 HS 品类</strong>
            <small>鲜冷、其他鲜菌与加工保藏</small>
          </div>
          <div>
            <span>更新体系</span>
            <strong>日度 + 月度 + 年度</strong>
            <small>渠道、官方统计与贸易基线互证</small>
          </div>
        </section>
        <section className="insight-section-head">
          <div>
          ...
          {countries.map(([code, name]) => {
            ...
            return (
              <article key={code}>
                ...
                <h3>{name}</h3>
                <strong>
                  {rows.length ? money(total) : hasPrice ? "查看价格" : "查看市场"}
                </strong>
                <p>
                  {rows.length
                    ? `${rows.length} 个菌类贸易品类`
        ...
        <section className="detail-card-grid insight-entry-grid">
          <article>
            ...
            <h2>目标市场需求画像</h2>
            <p>按国家查看贸易、价格和供应结构。</p>
            <a href="/insights/country">进入国别分析 →</a>
          </article>
          <article>
            <span>贸易分析</span>
            <h2>规模与来源结构</h2>
            <p>查看 HS 品类金额、来源构成和可信度。</p>
            <a href="/insights/trade">进入贸易分析 →</a>
          </article>
          <article>
            <span>渠道地图</span>
            <h2>平台与覆盖品类</h2>
            <p>查看已纳入价格观察的公开渠道。</p>
            <a href="/insights/channels">进入渠道地图 →</a>
          </article>
        </section>
      </main>
    </ProductShell>
