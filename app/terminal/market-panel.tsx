...
const dailyPrices = [
  { date:"2026-08-11", country:"KZ", city:"阿拉木图", product:"鲜双孢菇", channel:"电商/商超", price:"2,730–3,300 ₸/kg", source:"Arbuz / Carefood", url:"https://arbuz.kz/" },
  { date:"2026-08-11", country:"KZ", city:"阿拉木图", product:"鲜平菇", channel:"批发", price:"1,300–1,500 ₸/kg", source:"Flagma", url:"https://flagma.kz/" },
  { date:"2026-08-10", country:"UZ", city:"塔什干", product:"鲜双孢菇", channel:"电商/零售", price:"60,000–78,000 сум/kg", source:"OLX / Tegen", url:"https://www.olx.uz/" },
  { date:"2026-08-11", country:"KG", city:"比什凯克", product:"鲜双孢菇", channel:"商超零售", price:"520 сом/kg", source:"Globus Online", url:"https://globus-online.kg/ru-kg/good/9905ed980f9d469888dadc3efc68b6fe000200010000" },
  { date:"2026-08-11", country:"KG", city:"比什凯克", product:"鲜双孢菇 300g", channel:"本地电商", price:"128.70 сом/300g", source:"O!Market", url:"https://market.o.kg/bishkek/produkty-pitanija/ovoschi-frukty/product/4450008b-61fc-4fb7-839f-c4bdd5669c5c/griby-shampinony-1kg" },
  { date:"2026-08-11", country:"TJ", city:"杜尚别", product:"鲜双孢菇", channel:"即时零售", price:"86.30 сомони/kg", source:"Zudbiyor", url:"https://zudbiyor.tj/product/141" },
  { date:"2026-08-11", country:"TJ", city:"杜尚别", product:"鲜双孢菇 250g", channel:"商超电商", price:"29.90 сомони/250g", source:"Magnit.tj", url:"https://magnit.tj/product/show/18786" },
];
...
export default function MarketPanel({ onState }: { onState: (state: "loading" | "live" | "fallback") => void }) {
  ...
  return <>
    ...
    {grain === "daily" ? <section className="intel-table-panel"><div className="intel-panel-head"><div><span>DAILY MARKET OBSERVATIONS</span><h2>日度市场价格</h2></div><small>页面观察价 · 非成交价</small></div><div className="intel-table"><div className="intel-row daily-row head"><span>观察日</span><span>市场</span><span>商品</span><span>渠道</span><span>价格 / 来源</span></div>{prices.map(row => <div className="intel-row daily-row" key={`${row.date}-${row.country}-${row.product}-${row.channel}`}><span>{row.date}</span><span>{row.city}</span><span>{row.product}</span><span>{row.channel}</span><span><b>{row.price}</b><a href={row.url} target="_blank" rel="noreferrer">{row.source} ↗</a></span></div>)}{prices.length === 0 && <div className="daily-empty"><strong>{country === "TM" ? "土库曼斯坦：来源已发现，零售价待核验" : "当前筛选暂无合格记录"}</strong><p>{country === "TM" ? "已定位阿什哈巴德供应商与本地电商入口；现有蘑菇信息为出口询盘或餐饮成品价，暂不混入鲜菇日度零售价。" : "只展示同时具备商品、规格、价格和可追溯页面的记录。"}</p></div>}</div></section> : <>
