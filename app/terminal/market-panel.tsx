...
export default function MarketPanel({ onState }: { onState: (state: "loading" | "live" | "fallback") => void }) {
  ...
  return <>
    <section className="intel-hero terminal-controls"><div><span>DAILY EDIBLE MUSHROOM PRICES</span><h1>中亚五国每日食用菌细分品类价格终端</h1><p>跨国比较统一采用美元每公斤口径；原币、规格、换算过程和来源页面完整保留。未通过重量、分类或异常校验的记录不会进入行情汇总。</p></div><div className="asset-summary"><strong>{loading ? "—" : validRows.length || "—"}</strong><span>筛选内有效报价</span><strong>{loading ? "—" : new Set(validRows.map(r => r.platform.id)).size || "—"}</strong><span>有效覆盖平台</span></div></section>
    ...
    <section className="terminal-product-entry"><div><span>CLIENT PRODUCTS</span><h2>把公开观察转化为可执行的市场服务</h2><p>按国家、品类和渠道持续跟踪，也可获取定制数据交付。</p></div><div className="terminal-product-cards"><a href="/reports/daily"><b>每日行情简报</b><small>查看最新报告与关键变化</small></a><a href="/pricing"><b>监测订阅方案</b><small>选择国家、品类与更新频率</small></a><a href="/#contact"><b>定制数据与 API</b><small>提交目标市场和字段需求</small></a></div></section>
