...
def run():
 ...
 for Adapter,config in SOURCES:
  ...
  for row in rows:
   ...
   elif row.get("package_verified") is True and parse_package(package_text)["parse_status"]=="valid":
    package_source="page_structured_data"
   else:
 ...
 if not dry_run:
  ...
  for it in items:
   ...
   post_to_site("/api/ingest/snapshot",{"metric":"price_retail","country":it["country"],"source":it["platform"],"data":{"product_key":f'{it["platform"]}:{it["collection_point_id"]}:{it["platform_product_id"]}',"species_id":it["species_id"],"species_zh":labels.get("zh",it["species_id"]),"species_foreign":labels.get("ru") or labels.get("en"),"original_title":it["original_title"],"product_form":it["product_form"],"package_display":package_display,"package_source":it["package_source"],"platform_id":it["platform"],"platform_name":it["platform_name"],"status":"live","price_local":it["current_price"],"price_usd":it["price_usd"],"normalized_price_usd_per_kg":normalized_usd_per_kg,"currency":it["currency"],"observed_at":today,"retrieved_at":it["observed_at"],"source_url":it["product_url"]}})
