...
Uzum 全站对非白名单出口 IP 返回验证码(captcha_smart),本地直连不可用;
在 GitHub Actions(有 PROXY_BASE / 白名单出口)下走渲染通道可达。
...
CAPTCHA = re.compile(r"captcha|проверка|подтвердите, что вы не робот", re.I)
