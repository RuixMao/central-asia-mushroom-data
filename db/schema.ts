...
export const priceObservations=sqliteTable("price_observations",{id:text("id").primaryKey(),productId:text("product_id").notNull().references(()=>products.id),observedAt:integer("observed_at",{mode:"timestamp_ms"}).notNull(),observationDate:text("observation_date").notNull(),currentPrice:real("current_price"),regularPrice:real("regular_price"),promotionPrice:real("promotion_price"),currency:text("currency").notNull(),packageValue:real("package_value"),packageUnit:text("package_unit"),normalizedQuantityKg:real("normalized_quantity_kg"),normalizedPricePerKg:real("normalized_price_per_kg"),priceUsd:real("price_usd"),usdRateLocalPerUsd:real("usd_rate_local_per_usd"),fxSource:text("fx_source"),fxTimestamp:text("fx_timestamp"),inStock:integer("in_stock",{mode:"boolean"}),availabilityText:text("availability_text"),rawPriceText:text("raw_price_text"),sourceUrl:text("source_url").notNull(),sourceType:text("source_type").notNull(),pageFingerprint:text("page_fingerprint"),collectionStatus:text("collection_status").notNull(),validationStatus:text("validation_status").notNull(),createdAt:integer("created_at",{mode:"timestamp_ms"}).notNull()},t=>({daily:uniqueIndex("idx_prices_product_date").on(t.productId,t.observationDate),date:index("idx_prices_date").on(t.observationDate)}));
